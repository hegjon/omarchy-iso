...
# against a stub of the library that records every call. Also the package
# target / runtime package list helpers.
...
for fn in fs_perform_filesystem_operations installer_mount_ordered_layout installer_set_mirrors \
          ...
          mount_offline_package_cache unmount_offline_package_cache configure_limine_boot install_early_packages \
...
'arch_load_config;fs_perform_filesystem_operations;installer_mount_ordered_layout;installer_set_mirrors live;mount_offline_package_cache;mask /;installer_minimal_installation --no-mkinitcpio;installer_set_mirrors on_target;installer_setup_swap zstd;drop_archinstall_zram_conf;install_early_packages;configure_limine_boot;installer_create_users;applications_install;installer_add_additional_packages omarchy base-pkg;unmask /;unmount_offline_package_cache;installer_set_timezone UTC;installer_activate_time_synchronization;installer_set_user_password root $y$hash;installer_genfstab;installer_finish;'

section 'invariants'
check 'mkinitcpio deferred to the final UKI build' contains "$(steps)" 'installer_minimal_installation --no-mkinitcpio'
check 'limine set up after early packages, before useradd' test "$(calls | grep -n 'install_early_packages\|configure_limine_boot\|installer_create_users' | cut -d: -f1 | tr '\n' ' ')" == '11 12 13 '
...
check 'live hooks unmasked after the last pacstrap' test "$(calls | grep -n 'installer_add_additional_packages\|^unmask' | cut -d: -f1 | tr '\n' ' ')" == '15 16 '
...
# ── package targets / runtime package list ───────────────────────────────────
...
section 'runtime package list'
unset -f runtime_package_list
source "$ORCHESTRATOR/install.sh" # the real one again
printf '# base\nomarchy\nomarchy-settings\nlimine\ngit\n\nhyprland\nhyprland\n  waybar  \n' >"$OMARCHY_ISO_SHARE/omarchy-base.packages"
check 'runtime first, early and duplicate packages dropped' eq "$(runtime_package_list | tr '\n' ' ')" 'omarchy hyprland waybar '
