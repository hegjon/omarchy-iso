...
root_image_stream="$iso_payload_dir/omarchy-root.btrfs"
...
# One manifest over everything the install reads off the medium: the root image
# stream and every file in the offline mirror. omarchy-root-image-verify checks
# it at boot and the installer's pre-flight phase takes that verdict, so a badly
# flashed USB fails the install while it is still free to fail -- and, the
# mirror being many files, `sha256sum --check` names the file that failed, which
# is what the media diagnosis needs to tell a bad write from a bad read.
# Signatures and the repo databases are covered too; the databases' own
# %SHA256SUM% entries are not.
#
# The root image goes first: it is two thirds of the bytes and one file, so the
# hash progress the dashboard draws tracks it directly.
(
  cd "$iso_payload_dir" &&
    {
      printf '%s\n' "${root_image_stream##*/}"
      find "${mirror_files_dir##*/}" -maxdepth 1 -type f -print | sort
    } |
    xargs -r -d '\n' sha256sum >sha256sums
)
echo "Payload checksums: $(grep -c . "$iso_payload_dir/sha256sums") entries in sha256sums"
...
# install -- forever. Budget a floor of 2 MiB/s over everything the manifest
# covers (a USB 2.0 port sustains ~30 MB/s; the margin covers the idle-class
...
payload_bytes=$(du -sb --apparent-size "$root_image_stream" "$mirror_files_dir" | awk '{ total += $1 } END { print total }')
...
# Generated by build-iso.sh: 2 MiB/s floor over the $payload_bytes bytes the
# sha256sums manifest covers (root image stream + offline mirror).
[Service]
